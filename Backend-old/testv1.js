...
import { SmartSessionsController } from "@reown/appkit-experimental/dist/esm/src/smart-session/controllers";//  @reown/appkit-experimental/dist/esm/src/smart-session/controllers/SmartSessionsController';
import { assertWalletGrantPermissionsResponse, extractChainAndAddress, isValidSupportedCaipAddress } from '../../src/smart-session/helper/index.js';
...
describe('grantPermissions', () => {
    ...
    it('should successfully grant permissions and invoke required methods', async () => {
        ...
        expect(CosignerService.prototype.addPermission).toHaveBeenCalledWith('eip155:1:0x1234567890123456789012345678901234567890', mockRequest);
    ...
    it('should throw an error for unsupported namespace', async () => {
        ...
        await expect(SmartSessionsController.grantPermissions(mockRequest)).rejects.toThrow(ERROR_MESSAGES.UNSUPPORTED_NAMESPACE);
    });
    it('should throw an error when grantPermissions returns null', async () => {
        ...
        await expect(SmartSessionsController.grantPermissions(mockRequest)).rejects.toThrow(ERROR_MESSAGES.NO_RESPONSE_RECEIVED);
    });
    it('should handle network errors', async () => {
        vi.mocked(CosignerService.prototype.addPermission).mockRejectedValue(new Error('Network error'));
        await expect(SmartSessionsController.grantPermissions(mockRequest)).rejects.toThrow('Network error');
    });
    it('should throw an error for invalid chainId format', async () => {
        const invalidRequest = { ...mockRequest, chainId: '1' };
        await expect(SmartSessionsController.grantPermissions(invalidRequest)).rejects.toThrow(ERROR_MESSAGES.INVALID_CHAIN_ID_FORMAT);
    });
    it('should throw an error for invalid expiry', async () => {
        const invalidRequest = { ...mockRequest, expiry: -1 };
        await expect(SmartSessionsController.grantPermissions(invalidRequest)).rejects.toThrow(ERROR_MESSAGES.INVALID_EXPIRY);
    });
    it('should throw an error for empty permissions', async () => {
        const invalidRequest = { ...mockRequest, permissions: [] };
        await expect(SmartSessionsController.grantPermissions(invalidRequest)).rejects.toThrow(ERROR_MESSAGES.INVALID_PERMISSIONS);
    });
    it('should throw an error for invalid policies', async () => {
        const invalidRequest = { ...mockRequest, policies: {} };
        await expect(SmartSessionsController.grantPermissions(invalidRequest)).rejects.toThrow(ERROR_MESSAGES.INVALID_POLICIES_TYPE);
    });
    it('should throw an error for invalid signer', async () => {
        const invalidRequest = { ...mockRequest, signer: null };
    ...
    it('should throw an error for invalid signer type', async () => {
        const invalidRequest = { ...mockRequest, signer: { type: {}, data: {} } };
        await expect(SmartSessionsController.grantPermissions(invalidRequest)).rejects.toThrow();
    });
    ...
    it('should throw an error when CosignerService addPermission fails', async () => {
        vi.mocked(CosignerService.prototype.addPermission).mockRejectedValue(new Error('Cosigner error'));
        await expect(SmartSessionsController.grantPermissions(mockRequest)).rejects.toThrow('Cosigner error');
    });
    it('should throw an error when ConnectionController grantPermissions fails', async () => {
        ...
        await expect(SmartSessionsController.grantPermissions(mockRequest)).rejects.toThrow('Connection error');
    });
    ...
    it('should throw an error for an invalid response in assertWalletGrantPermissionsResponse', () => {
        ...
        expect(() => assertWalletGrantPermissionsResponse(invalidResponse)).toThrow(ERROR_MESSAGES.INVALID_GRANT_PERMISSIONS_RESPONSE);
    });
});
//# sourceMappingURL=grantPermissions.test.js.map